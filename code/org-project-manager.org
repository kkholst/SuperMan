...
(defun org-pro-update-status ()
  ...
  (save-excursion 
    ...
    (goto-char (point-min))
    (re-search-forward "\\* Documents")
    (org-columns)
    )
)

(defvar org-pro-narrow-on nil)
(make-variable-buffer-local 'org-pro-narrow-on)
(defun org-pro-narrow-and-column () 
  (interactive)
  (setq org-pro-narrow-on (not org-pro-narrow-on))
  (if org-pro-narrow-on
      (progn 
...
;;			   (local-set-key (kbd "C-x c") 'org-pro-narrow-and-column)))
#+END_SRC
