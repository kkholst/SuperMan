...
Projects are defined in the file /org-pman-file/
...
"Subheading level at which projects are defined in `org-pman-file'.")
#+END_SRC
    ...
    (add-hook 'find-file-hooks 
              (lambda ()
                (let ((file (buffer-file-name)))
                  (when (and file (equal file (expand-file-name org-pman-file)))
                    (org-pman-minor-mode)))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :export code
  ...
  (defun org-pman-parse-projects (&optional all)
             ...
             (save-excursion
               ...
               (set-buffer (find-file-noselect org-pman-file))
               (save-buffer)
               (goto-char (point-min))
...
(defun org-pman-create-project (&optional project ask)
  ...
  (let ((pro (assoc project org-pman-project-alist)))
    (when pro
      (let ((dir (concat (org-pman-get-location pro) (car pro)))
...
(defun org-pman-new-project (&optional nickname category)
                            ...
                            (let* ((nickname (or nickname (read-string "Project name (short) ")))
                              ...
                              (let ((org-capture-templates
                                     `(("p" "Project" plain
                                      (file+headline org-pman-file ,category)
...
;;                                  (interactive)
                                  (let* ((pro (or project org-pman-select-project))
                                         (dir (concat (org-pman-get-location pro) (car pro)))
...
#+BEGIN_SRC  emacs-lisp :export code
(setq org-pman-config "INDEX | LOCATION / TIMELINE | TODO")
;; could be 
;; (setq org-pman-config "recent.org / *R* | TODO")
(setq org-pman-config-action-alist '(("INDEX" . (lambda (project) (find-file (org-pman-get-index project))))
                                ("TODO" . org-pman-todo)
                                ("TIMELINE" . org-pman-timeline)
                                ("LOCATION" . org-pman-location)
                                ("recent.org" . org-pman-recent-org)
...
(defun org-pman-read-config (config)
 ...
 (let* ((vlist  (split-string config "[ \t]+|[ \t]+"))
   ...
   clist))
(defun org-pman-set-config (&optional project config)
 (interactive)
   (let* ((pro (or project org-pman-current-project (org-pman-select-project)))
          (conf (or config (org-pman-get-config pro) org-pman-config))
          (window-config-list (org-pman-read-config conf))
          swindow)
   ;; create windows
   (delete-other-windows)
...
#+BEGIN_SRC emacs-lisp :export code
   (defvar org-pman-switch-always t "If nil 'org-pman-switch-to-project' will
     ...
     (defun org-pman-list-files (dir ext &optional sort)
     ...
     (defun org-pman-layout (&optional pos)
                   (delete-other-windows)
  ...
  (defun org-pman-location (project)
      (find-file (concat (org-pman-get-location project) (car project))))
  
    (defun org-pman-timeline (project)
             (let (tbuf)
  ...
  (defun org-pman-recent-org (project)
           (car (org-pman-list-files (concat (org-pman-get-location project) (car project)) "^[^\\.].*\\.org$" "time")))
         
  (defun org-pman-todo (project)
             (let (tbuf)
             (save-window-excursion
               (let* ((index (org-pman-get-index project))
                      (location (concat (org-pman-get-location project) (car project)))
     ...
     (defun org-pman-find-project (project)
      ...
      ;; (let* ((index (org-pman-get-index project))
               ...
               (defvar org-pman-human-readable-ext "^[^\\.].*\\.org\\|\\.[rR]\\|\\.tex\\|\\.txt\\|\\.el$" "Extensions of human readable files")
               (defun org-pman-open-recent (&optional project action)
                         (interactive)
                         (let* ((pro (or project org-pman-current-project (org-pman-select-project)))
                                (loc (concat (org-pman-get-location pro) (car pro)))
                                (tmplist (if (require 'file-list nil t)
                                             (file-list-select-internal nil org-pman-human-readable-ext nil nil loc nil 'dont)))
           ...
           (defun org-pman-get-config (project)
                           ...
                           (defun org-pman-get-git-location (project)
                                                   (or (cdr (assoc "git-location" (cadr project)))
                                                       (concat (org-pman-get-location project) (car project))))
...
(defun org-pman-set-publish-alist ()
  ...
  (let ((p-alist org-pman-project-alist))
    (while p-alist
      (let* ((pro  (car p-alist))
	     (nickname (car pro))
	     (base-directory (concat (org-pman-get-location pro) (car pro)))
	     (export-directory
	      (concat base-directory
		      org-pman-export-subdirectory))
	     (public-directory
	      (or (org-pman-get-publish-directory pro)
