...
(defun org-pro-move-project (&optional project)
  ...
  (let* ((pro (or project (org-pro-select-project)))
    ...
    (when (yes-or-no-p (concat "Move " dir " to " target "? "))
       ...
       (org-set-property "INDEX" (or new-index (replace-in-string index (file-name-directory dir) (file-name-directory target))))
       (save-buffer))))
;;     (save-excursion
...
(defun org-pro-git-add-file (file dir)
  "Add file FILE to git repository DIR."
  (with-output-to-temp-buffer "*Git log*" 
    (shell-command (concat "cd " dir "; git add " file))))

;; TODO
;;(defun org-pro-update-document-status ()
;;  (interactive)
;;  (org-back-to-heading t)
;;  (let ((file (org-entry-get nil "filename"))
;;        (old-stat (org-entry-get nil "Git-status")))
;;    (cond (string= stat "changed"))))
    ...
    (let* ((config (or org-pro-sticky-config org-pro-default-config "INDEX"))
	   ...
	   (filed-config (when (file-exists-p config-file)
			   (save-window-excursion
			     ...
			     (replace-in-string (replace-in-string (buffer-string) "[\n\t ]+$" "") "\n" " : "))))
...
*** Capturing documents
#+BEGIN_SRC  emacs-lisp :export code
(add-to-list 'org-capture-templates `(,(concat org-pro-capture-prefix "d") "Add document" plain
				      (function org-pro-goto-project-workflow) "\n*** DOCS [[%(read-file-name \"Document file: \")][%(read-string \"Document name: \")]] \n:PROPERTIES:\n:CaptureDate: %T\n:END:"))
