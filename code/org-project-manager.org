...
(defun org-pro-git-log-at-point (arg &optional file searchstring decorationonly)
  (interactive "p")
  (when org-pro-git-log-mode (error "Already in log-buffer"))
  (let* ((limit (or arg org-pro-log-limit))
	 ...
	 (file-name-relative (file-relative-name file gitpath))
	 (gitlog (shell-command-to-string
		  (concat "cd " gitpath "; " org-pro-cmd-git  " log --pretty=\"%h#&#%s#&#%ad#&#%an#&#%d\" --date short " gitsearch " "
			  (if limit (concat "-n " (int-to-string limit))) " -- " file-name-relative)))
	 item val
	 (logbuf (concat "*" (file-name-nondirectory file)  " git log")))
    (if (string= gitlog "")
	(error "File not (not yet) git controlled.")
      ...
      (org-columns)
      (org-columns-content)
      (setq org-pro-log-assoc-file file-name-relative)
...
** Capture: Adding information to projectsy
*** Choose a prefix
#+BEGIN_SRC  emacs-lisp :export code
