    ...
    (let* ((file (org-pro-filename-at-point))
     ...
     (if (string-match "^fatal" git-status)
      ...
      (if (or  (string= git-status "")  (string= git-status "E"))
        ...
        (setq git-last-commit (shell-command-to-string (concat "cd " dir "; git log  -1 --pretty=format:\"%ad\" " file))))
     ...
     (unless (org-pro-entry-get nil "GitInit")
        (org-set-property
         ...
         (replace-regexp-in-string
          "[ \n\t]*$" ""
          (shell-command-to-string (concat "git log --pretty=format:\"%ad\" --reverse " file "  | head -1")))))
