...
(defun org-pro-git-log-at-point (&optional file)
  (interactive)
  (widen)
  (let ((gitlog) (result) (item) (val) (myfile))
    (setq myfile (or file  
	  (file-relative-name (org-pro-filename-at-point))))
    (setq gitlog
	(split-string (substring 
	 (shell-command-to-string 
	  (concat "git log --pretty=\"%h#&#%s#&#%ad#&#%an\" --date short -- " myfile))
	 0 -1) "\n"))
  ;;  (generate-new-buffer "*org-pro-git-scratch*")
  (switch-to-buffer "*org-pro-git-scratch*")
  (org-columns-quit)
  (erase-buffer)
  (insert-string (concat "* Git Log (" myfile ")\n:PROPERTIES:\n:COLUMNS: %40ITEM(Comment) %Date %15Author %8Hash \n:FILE: " myfile "\n:END:\n"))
  (setq result "")
...
(defun org-pro-git-revision-at-point ()
  ...
  (let ((myhash (org-pro-entry-get nil "hash" nil))
	(myfile (org-pro-entry-get nil "file" t))  
	(mystr))
    (setq mystr (shell-command-to-string 
		 (concat "git show " myhash ":" myfile)))
