...
(defun org-pro-read-git-date (git-date-string &optional no-time)
  ...
  (defun org-pro-git-last-commit (file)
    (interactive)
    (let* ((date (org-pro-read-git-date
                  (shell-command-to-string
                   (concat "git log --date=local -1 --pretty=format:\"%ad\" " file))))
           (mess (shell-command-to-string
		  (concat "git log -1 --pretty=format:\"%s\" " file))))
      (concat date " " mess)))
  ...
  (defun org-pro-git-status-file-at-point ()
	...
	(if (or  (string= git-status "")  (string= git-status "E"))
	  ...
	  (setq git-last-commit (org-pro-git-last-commit file dir) ))
