...
#+BEGIN_SRC emacs-lisp :export code 
  ...
  (defun org-pro-help-mode ()
    (interactive)
    (split-window-vertically)  
    (other-window 1)
    (switch-to-buffer org-pro-helpbuf)
    (toggle-read-only -1)
    (erase-buffer)
    (insert-string "'<ret>':\t\t Open file (or revision) at point\n")
    (insert-string "'l':    \t\t Show git log\n")
    (insert-string "'L':    \t\t Show git log for tagged revisions\n")
    (insert-string "'u':    \t\t Update git status\n")
    (insert-string "'b':    \t\t Blame\n")
    (insert-string "'S':    \t\t Search for revision containing a regular expression\n")
    (insert-string "'D':    \t\t Show difference between revision at point and HEAD\n")
    (insert-string "'h':    \t\t Open this help window\n")
    (insert-string "'t':    \t\t Alter tag (empty string to remove)\n")
    (insert-string "'q':    \t\t Quit view mode\n")
    (toggle-read-only 1)
    (other-window -1) 
  )
    
        (org-defkey org-columns-map "\r" 'org-pro-column-action) ;; Return is not used anyway in column mode
        (org-defkey org-columns-map "l" 'org-pro-git-log-at-point) 
        (org-defkey org-columns-map "L" (lambda () (interactive) (org-pro-git-log-at-point nil nil t)))
        (org-defkey org-columns-map "S" 'org-pro-git-search-at-point)
        (org-defkey org-columns-map "b" 'org-pro-git-blame-at-point)
        (org-defkey org-columns-map "t" 'org-pro-git-tag-at-point)
        (org-defkey org-columns-map "h" 'org-pro-help-mode)
        (org-defkey org-columns-map "D" (lambda () (interactive) (org-pro-git-revision-at-point t)))

        (org-defkey org-columns-map "q" (lambda ()          
                                          (interactive)
                                          (org-columns-quit)
                                          (if (string= (buffer-name) org-pro-viewbuf) (kill-buffer org-pro-viewbuf))
                                          (if (buffer-exists org-pro-helpbuf) (kill-buffer org-pro-helpbuf))
                                          (delete-frame)
                                          ))
    
        (org-defkey org-columns-map "u" '(lambda () 
                                           (interactive)
                                           (if (org-pro-entry-get nil "hash" nil)
                                               (org-pro-git-revision-at-point t)
                                             (progn 
...
#+RESULTS[0dd16cc781a4945e403eff5045384b7ce874252b]:
: org-pro-git-update-project

*** Exploiting column view
...
#+BEGIN_SRC emacs-lisp :export code    
  ...
  (defun org-pro-column-action ()
    ...
    (let* (
      ...
      (cond ((string= prop "ITEM")
             ...
             (setq tempfrm (selected-frame))
               (if (re-search-forward ":Hash:" nil t)
                   (progn 
                     (widen)
                     (org-pro-git-revision-at-point)
                     (setq tempstr (buffer-name))
                     (delete-frame tempfrm)
                     (switch-to-buffer tempstr)
                     (let ((buffer-file-name (expand-file-name (buffer-name)))) (normal-mode))
                     ))
               (if (re-search-forward ":FileName:" nil t)
                   (progn 
                     (widen)
                     (setq tempstr (file-name-nondirectory (org-pro-filename-at-point)))
  ...
  (defvar org-pro-help-always-on t)
  (defvar org-pro-viewbuf "*org-pro-view-buffer*")
  (defvar org-pro-helpbuf "*org-pro-help-buffer*")
  (defvar org-pro-scratchbuf "*org-pro-git-scratch*")
  (defvar org-pro-viewframename "Project Manager View Frame")
  ;; (defvar org-pro-view-mode nil)
  ;; (make-variable-buffer-local 'org-pro-view-mode)
  (defun org-pro-view-mode (&optional status) 
    (interactive)
    (let ()
      ;; (setq org-pro-view-mode (or status (not org-pro-view-mode)))
      (if (buffer-exists org-pro-viewbuf) (progn
                                 (switch-to-buffer org-pro-viewbuf)
                                 (org-columns-quit)
                                 (kill-buffer org-pro-viewbuf))
        )
     
      ;; (mapcar (lambda (element)
      ;;          (let ((key (car element)))
      ;;            (string= org-pro-viewframename key)))
      ;;         (make-frame-names-alist))
    
      (select-frame (make-frame `((name . ,org-pro-viewframename))))
      (set-frame-size (selected-frame) 90 60)
      (make-indirect-buffer (buffer-name) org-pro-viewbuf)
      (switch-to-buffer org-pro-viewbuf)
  ;;    (add-hook 'kill-buffer-hook (lambda () 
  ;;                                (if (get-buffer org-pro-helpbuf) (kill-buffer org-pro-helpbuf))) t)
      (org-mode)
      (org-pro-minor-mode)
      ...
      (org-columns-quit)
      (org-columns)
      (if org-pro-help-always-on (org-pro-help-mode))
      ))
  ...
  (defun org-pro-git-log-at-point (&optional file searchstring decorationonly)
    (interactive)
    (widen)
    (let ((gitlog) (result) (item) (val) (myfile) (gitpath) (gitsearch))
      (setq myfile (file-relative-name (or file  
                                           (org-pro-filename-at-point))))
      (setq gitsearch (if searchstring (concat " -G" searchstring) ""))
      (setq gitpath (replace-regexp-in-string myfile "" (expand-file-name myfile)))
      (setq gitlog
            (split-string (substring 
                           (shell-command-to-string 
                            (concat org-pro-cmd-git " log --pretty=\"%h#&#%s#&#%ad#&#%an#&#%d\" --date short " gitsearch " "
                                    (if org-pro-log-limit (concat "-n " (int-to-string org-pro-log-limit))) " -- " myfile))
                           0 -1) "\n"))
      ;;  (generate-new-buffer "*org-pro-git-scratch*")
      (switch-to-buffer org-pro-scratchbuf)
      (org-columns-quit)
      (erase-buffer)
      ...
      (loop for x in gitlog
            ...
            (if (or (not decorationonly) (nth 4 val)) (insert-string item)))
  ...
  (defun org-pro-git-search-at-point (&optional file)
     (interactive)
     (org-pro-git-log-at-point file (read-string "Search string: "))
  )
    ...
    (let ((myhash (org-pro-entry-get nil "hash" nil))
      ...
      (if (string-equal tag "")
          (progn 
            ...
            (shell-command (concat "cd " mypath ";" org-pro-cmd-git " tag -d " mytag))
            )
        (shell-command (concat "cd " mypath ";" org-pro-cmd-git " tag -a " tag " " myhash " -m \"\"")))
      ) 
    (save-excursion
      (goto-char (point-min))
  ...
  (defun org-pro-git-revision-at-point (&optional diff)
    ...
    (let ((myhash (org-pro-entry-get nil "hash" nil))
          ...
          (fileabs)(filehash)
          (mystr))
      (setq fileabs (concat mypath myfile))
      (setq filehash (concat myhash "_" myfile))
      (setq mystr (shell-command-to-string 
                   (concat "cd " mypath ";" org-pro-cmd-git " show " myhash ":" myfile)))
      (if diff (progn 
                 (find-file fileabs)))
      ;;(set-buffer (find-file-noselect fileabs))))
      (switch-to-buffer-other-window filehash)
      (erase-buffer)  
      (insert-string mystr)
      (normal-mode) ;; Get default major-mode 
  ;;    (text-mode)
      (if diff (ediff-buffers (file-name-nondirectory myfile) filehash))
      ))
  
#+END_SRC

#+RESULTS[8de7a830b00c23976dfcf1284223b354aa34adb6]:
: org-pro-git-revision-at-point
...
(defun org-pro-find-index (project)
  (let* ((index (org-pro-get-index project)))
	...
	(make-directory (file-name-directory index) 'with-parents))
      (make-directory (file-name-directory index) 'with-parents))
    (find-file index)
   ;; FIXME: this should to be removed when document-view is
   ;; in a different buffer
;;    (org-pro-view-mode nil)))
))
...
;;          (re-search-backward (format org-complex-heading-regexp-format (regexp-quote head)))
))))
  ...
  (defun org-pro-insert-list-files (&optional postfix)
    (interactive)
    (let ((myfiles (org-pro-list-files "." 
                                       (or postfix (concat (read-string "Postfix: ") "$")) nil)))
      (insert-string "\n")
      (loop for x in myfiles
            do 
            (insert-string (concat "*** " (file-relative-name x) "\n:PROPERTIES:\n:FileName: [[" (file-relative-name x) "]]\n:END:\n")))
      ))
