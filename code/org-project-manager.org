  ...
  (defun org-pro-delete-project (&optional project)
    ...
    (let* ((pro (or project (org-pro-select-project)))
      ...
      (when (yes-or-no-p (concat "Really remove project " (car pro) "? "))
        ...
        (find-file org-pro-file)
        (goto-char (point-min))
        (re-search-forward (concat "*[ \t]+.*" (car pro)) nil t)
        (org-mark-subtree)
...
(defun org-pro-git-revision-at-point (&optional diff)
  ...
  (let ((myhash (org-pro-entry-get nil "hash" nil))
    ...
    (setq filehash (concat myfile "." myhash))
...
#+RESULTS[a9d3552e32983262b0035c1b310548054498d964]:
: org-pro-git-revision-at-point
...
(defun org-pro-browse-this-file ()
  (interactive)
  (let* ((bf (buffer-file-name (current-buffer))))
    (message (concat "file:///" (file-name-sans-extension bf) ".html"))
    (browse-url (concat "file:///" (file-name-sans-extension bf) ".html"))))


(defvar org-pro-public-server-home "" "Place on the web where pages are published.")
(defun org-pro-browse-public-version ()
  (interactive)
  (let* ((bf (buffer-file-name (current-buffer)))
	 (html-file
