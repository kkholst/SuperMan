...
(defun org-pro-filename-at-point ()
  (let* ((file-or-link (org-pro-entry-get nil "filename" nil)))
...
(defun org-pro-read-git-date (git-date-string &optional no-time passiv)
  "Transform git date to org-format"
  (with-temp-buffer
    (org-insert-time-stamp 
     (date-to-time git-date-string) (not no-time) passiv)))
;;      (set-time-zone-rule t) ;; Use Universal time.
;;      (prog1 (format-time-string "%Y-%m-%d %T UTC" time)
;;        (set-time-zone-rule nil))))

(defun org-pro-git-get-commit (arg file &optional dir passiv)
  (interactive)
	 ...
	 (date (org-pro-read-git-date
		(shell-command-to-string
		 (if (string= arg "first")
		   ...
		   (concat "git log --date=local -" arg " --pretty=format:\"%ad\" " file))) nil passiv))
...
(defun org-pro-git-status-file-at-point ()
  ...
  (let* ((file (org-pro-filename-at-point))
	 ...
	 (git-status (shell-command-to-string (concat "cd " dir "; git status --porcelain " file)))
	 git-last-commit
	 status)
	...
	(if (string-match "^fatal" git-status)
      ...
      (if (or  (string= git-status "")  (string= git-status "E"))
	...
	(setq git-last-commit (org-pro-git-get-commit "1" file dir) ))
      (cond ((string= git-status "?")
	    ...
	    (t (setq status "Unkown")))
      (unless (org-pro-entry-get nil "GitInit")
	(org-set-property "GitInit" (org-pro-git-get-commit "first" file dir 'passiv)))
...
(defun org-pro-git-commit-file-at-point ()
  "Add or update file FILE to git repository DIR."
  (interactive)
  (let* ((file (org-pro-filename-at-point))
	 (dir (if file (file-name-directory file)))
...
(defun org-pro-goto-project-documents ()
  (interactive)
  (or (org-pro-goto-project nil "Documents" 'create)))
