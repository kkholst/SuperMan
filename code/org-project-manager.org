...
(define-key org-pro-view-mode-map "h" 'org-pro-help-mode)
...
(define-key org-pro-view-mode-map "u" '(lambda () 
					 (interactive)
...
(defvar org-pro-log-limit 5)
(defun org-pro-git-log-at-point (arg &optional file searchstring decorationonly)
  (interactive "p")
  (let* ((limit (or arg org-pro-log-limit))
	 (file (expand-file-name (or file (org-pro-filename-at-point))))
	 (gitsearch (if searchstring (concat " -G" searchstring) ""))
	 (gitpath (org-pro-git-toplevel file))
	 (gitlog (split-string (substring 
				(shell-command-to-string 
				 (concat org-pro-cmd-git " log --pretty=\"%h#&#%s#&#%ad#&#%an#&#%d\" --date short " gitsearch " "
					 (if limit (concat "-n " (int-to-string limit))) " -- " file))
				0 -1) "\n"))
	 item val
...
(defun org-pro-git-revision-at-point (&optional diff)
  ...
  (let* ((myhash (org-pro-entry-get nil "hash" nil))
	 ...
	 (mypath (org-pro-entry-get nil "gitpath" t))
	 (fileabs (concat mypath myfile))
	 (filehash (concat myhash "_" myfile))
	 (mystr (shell-command-to-string 
