  ...
  (defun org-pro-parse-projects (&optional all)
             ...
             (save-excursion
               ...
               (while (org-pro-forward-project)
                   (let* ((loc (replace-in-string (or (org-entry-get nil "LOCATION" 'inherit) org-pro-default-directory) "[\t ]+$" ""))
...
#+BEGIN_SRC emacs-lisp :export code
  ...
  (defun org-pro-set-frame-title ()
    (let ((old-format (cadr (split-string frame-title-format "Project:[ \t]+[^ \t]+[ \t]+"))))
    (setq frame-title-format
          (concat "Project: " (or (car org-pro-current-project) "No active project") " " old-format))))
