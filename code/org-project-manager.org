...
#+BEGIN_SRC  emacs-lisp :export code
(setq org-pro-config "INDEX / LOCATION | TIMELINE / TODO : recent.org | *R*")
;; could be 
...
(defun org-pro-save-config (&optional config project)
  ...
  (let ((conf (or config (org-pro-current-config)))
     ...
     (when (y-or-n-p (concat "Save current window configuration for project " (car pro) "? "))
...
(defun org-pro-get-config (project)
  (let* ((config-file  (concat (org-pro-get-location project) (car project) "/.org-pro-window-config"))
	(config (when (file-exists-p config-file)
			(save-window-excursion
			  (find-file config-file)
			  (replace-in-string (replace-in-string a "\n" " : ") "\n" " : "))))
