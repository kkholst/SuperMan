...
#+BEGIN_SRC emacs-lisp :export code
  ...
  (defvar org-project-manager (concat
                               (file-name-as-directory org-directory)
                                         "Projects.org")
  "Where the org-project-manager defines the projects. See the manual
...
#+BEGIN_SRC emacs-lisp :export code
  ...
  (defun org-project-manager-find-project (project)
      (let ((index (org-project-manager-get-index project))
            (location (org-project-manager-get-location project))
    ;;        (agenda (org-project-manager-get-agenda project))
    (org-agenda-custom-commands
          `(("x" "Project agenda"
             ((todo "TODO"  ((org-agenda-files `(,index)))))))))
    (delete-other-windows)
    (if index (find-file index))
   (push ?x unread-command-events)
   (call-interactively 'org-agenda)
   (org-agenda-redo)
   (split-window-horizontally)
    (other-window 1)
    (if location (find-file location))
