...
(defun org-pro-git-status-file-at-point ()
  ...
  (let* ((file (org-pro-filename-at-point))
	...
	(setq git-last-commit "")
      (setq git-last-commit (shell-command-to-string (concat "cd " dir "; git log  -1 --pretty=format:\"%ad %s\" " file))))
    ...
    (unless (org-pro-entry-get nil "GitInit")
      (org-set-property
       ...
       (replace-regexp-in-string
	"[ \n\t]*$" ""
	(shell-command-to-string (concat "cd " dir "; git log --pretty=format:\"%ad\" --reverse " file "  | head -1")))))
...
(defun org-pro-git-commit-file-at-point ()
  ...
  (let* ((file (org-pro-filename-at-point))
    ...
    (shell-command (concat "cd " dir "; git commit -m \"" message "\" "  file))))
...
;; (if doit
;; (shell-command (concat "cd " dir "; git add -uf;git commit -m \"" message "\"")))))
