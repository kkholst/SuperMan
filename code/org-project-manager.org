...
(defun org-pro-view-mode (&optional status) 
  (interactive)
  (let ((viewbuf "*org-pro-view-buffer*"))
    ;; (setq org-pro-view-mode (or status (not org-pro-view-mode)))
    (save-excursion 
    (if (get-buffer viewbuf) (progn
			       (switch-to-buffer viewbuf)
			       (org-columns-quit)
			       (kill-buffer viewbuf))
      )
    ) 
    (make-indirect-buffer (buffer-name) viewbuf)
    (switch-to-buffer viewbuf)
    (org-mode)
...
(defun org-pro-git-log-at-point (&optional file searchstring)
  (interactive)
  (widen)
  (let ((gitlog) (result) (item) (val) (myfile) (gitpath) (gitsearch))
    ...
    (setq gitlog
	  (split-string (substring 
			 (shell-command-to-string 
			  (concat "git log --pretty=\"%h#&#%s#&#%ad#&#%an#&#%d\" --date short " gitsearch " "
